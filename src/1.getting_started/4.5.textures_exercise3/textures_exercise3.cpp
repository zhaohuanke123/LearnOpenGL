...
int main() {
    ...
    // ------------------------------------------------------------------
    float vertices[] = {
        // 顶点位置          // 颜色              // 纹理坐标（注意：我们将纹理坐标更改为“放大”纹理图像）
    ...
    // 设置纹理环绕参数
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); // 注意：我们将容器的纹理环绕方式设置为 GL_CLAMP_TO_EDGE
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    // 设置纹理过滤参数
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); // 设置纹理过滤为最近邻，以便清楚地看到纹理元素/像素

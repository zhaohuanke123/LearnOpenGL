...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT); // 现在也清除深度缓冲区！
        ...
        // 创建变换矩阵
        glm::mat4 view = glm::mat4(1.0f); // 确保首先将矩阵初始化为单位矩阵
        glm::mat4 projection = glm::mat4(1.0f);
        projection = glm::perspective(glm::radians(fovRotate), (float) SCR_WIDTH / (float) SCR_HEIGHT, 0.1f, 100.0f);
        view = glm::translate(view, glm::vec3(0.0f, 0.0f, -3.0f));
        // 将变换矩阵传递给着色器
        ourShader.setMat4("projection", projection); // 注意：目前我们每帧都设置投影矩阵，但由于投影矩阵很少改变，通常最好只在主循环外部设置一次。
        ourShader.setMat4("view", view);

        // 渲染立方体
        // glBindVertexArray(VAO);
        // for (unsigned int i = 0; i < 10; i++) {
        //     // 计算每个物体的模型矩阵并在绘制前传递给着色器
